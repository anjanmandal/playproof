...
exports.buildInputImageContents = exports.buildInputImageContent = void 0;
 ...
 */
const getPublicBase = () => {
    if (env_1.env.PUBLIC_URL) {
        try {
            return new URL(env_1.env.PUBLIC_URL);
        }
        catch {
...
const resolveLocalUploadPath = (mediaUrl) => {
    try {
        if (mediaUrl.startsWith("/uploads/")) {
            return path_1.default.join(storage_1.mediaRoot, mediaUrl.replace("/uploads/", ""));
        }
        ...
        if (parsed.host === publicBase.host) {
            return path_1.default.join(storage_1.mediaRoot, parsed.pathname.replace("/uploads/", ""));
        }
    }
    catch {
 ...
 */
const buildInputImageContent = async (mediaUrl) => {
    if (!mediaUrl)
        return null;
    ...
    if (localPath) {
        try {
            ...
            const publicBase = getPublicBase();
            const relative = path_1.default.relative(storage_1.mediaRoot, localPath);
            return {
                type: "input_image",
...
exports.buildInputImageContent = buildInputImageContent;
/**
 * Convenience: build multiple image content parts (filters out nulls).
 */
const buildInputImageContents = async (mediaUrls) => {
    const parts = await Promise.all(mediaUrls.map((u) => (0, exports.buildInputImageContent)(u)));
    return parts.filter((p) => Boolean(p));
};
exports.buildInputImageContents = buildInputImageContents;
//# sourceMappingURL=openaiMedia.js.map